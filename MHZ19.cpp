...
void MHZ19::constructCommand(Command_Type commandtype, int inData, byte asemblecommand[])
{
    ...
    /* set command */
    asemblecommand[2] = commandtype; // assign command value
...
void MHZ19::constructCommand(Command_Type commandtype, int inData)
{
    ...
    constructCommand(commandtype, inData, asemblecommand);
...
{
    if (this->storage.constructedCommand[2] == Command_Type::RAWCO2)	// compare commands byte
        read(this->storage.responses.RAW, commandtype);				// returns error number, passes back response and inputs command

    else if (this->storage.constructedCommand[2] == Command_Type::CO2UNLIM)
        read(this->storage.responses.CO2UNLIM, commandtype);

    else if (this->storage.constructedCommand[2] == Command_Type::CO2LIM)
